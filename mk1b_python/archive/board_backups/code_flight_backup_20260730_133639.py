...
# --- per-channel continuity (pyro.pyroTest only returns a single bool) --------

def per_channel_continuity(p):
    # pyro_low energises the high-side; without it the sense divider reads 0.
    p.fire1.value = False
    p.fire2.value = False
    p.pyro_low.value = True
    time.sleep(0.05)
    p1_ok = p.sense1.value < 1000
    p2_ok = p.sense2.value < 1000
